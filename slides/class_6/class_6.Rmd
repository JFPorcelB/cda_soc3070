...
.pull-left[
```{r, echo=FALSE, fig.width=7, fig.height=6, warning=F, message=F}
...
- Cada $\hat{p} \sim \text{Normal}(\mu,\sigma) \quad \text{ con } \quad \mu = p \quad \text{ y }\quad \sigma = \sqrt{p(1-p)/n}$

--

- Cual es la distribución del ratio de dos normales, $\text{Normal}(\mu_{1},\sigma_{1})/ \text{Normal}(\mu_{2},\sigma_{2})$ ?
...
- Asintóticamente $\log \hat{RR} \sim \text{Normal}(\mu,\sigma)$, con parametros _estimados_ por: 

  -  $\quad \hat{\mu}: \log \hat{p}_{1} - \log \hat{p}_{2} \quad \text{ y }\quad \hat{\sigma} = \sqrt{1/n_{11} - 1/n_{1+} + 1/n_{21} - 1/n_{2+} }$
