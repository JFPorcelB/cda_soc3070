...
style_duo_accent(primary_color = "#4953A6", secondary_color = "#E5E68C" ,
...
plot <- ggplot(df, aes(x = prize)) +
  geom_bar(aes(y = ..prop..), fill = "#E5E68C", color = "black", width = 100) +
...
plot <- ggplot(data = mydata, mapping = aes(x = x, y = y)) +
  ...
  geom_bar(aes(fill = "#E5E68C"), color = "black", stat = "identity", position = "dodge", width = 1) +
...
plot <- ggplot(data = data.frame(p = 0), mapping = aes(x = p)) +
  ...
  geom_point(aes(x = 0.5, y = 1000/4), fill = "#E5E68C", shape=21, size = 8) +
...
plot <- ggplot(data = mydata, mapping = aes(x = x)) +
  # Binomial Bars with black borders and no space between bars
  geom_bar(aes(y = binom, fill = "#E5E68C"), color = "black", stat = "identity", position = "dodge", width = 1) +
...
data_coins %>% 
  ggplot(aes(x=factor(X))) + 
    ...
    scale_fill_manual(values = c("#E5E68C", "#E5E68C")) + 
...
plot + 
  stat_function(fun = binom_distrib, args = list(n= 100, k= 82), size=1.5, color = "#E5E68C") + 
  xlim(0,1) + 
...
parameter_space %>%
  ...
  geom_point(aes(x=m[which.max(m[,2]),1], y=m[which.max(m[,2]),2]), size=2.5, color="#E5E68C") +
  scale_color_gradient(low = "#4953A6", high = "#E5E68C") + 
  guides(fill=FALSE, color=FALSE) + 
