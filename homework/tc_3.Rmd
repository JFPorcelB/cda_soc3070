...
Para hacerlo, debes reemplazar la variable `mi_L` en el siguiente código por la expresión obtenida en la pregunta (1), usando `x` en lugar de $p$. Aquí ` mi_L = cos(-2*p)` sirve sólo a modo de ejemplo.

```{r, echo=TRUE, message=FALSE, warning=FALSE, fig.height=4, fig.width=5}
mydata <- data_frame(x = seq(from = 0, to = 1, by=0.01), mi_L = cos(-2*x) )
