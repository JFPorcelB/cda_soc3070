...
1. Calcula la probabilidad de votar por el NO para hombres y mujeres cuyos ingresos y apoyo al status-quo se encuentran en los valores medianos de la muestra.
...
2. Usa el método de Bootstrap para crear un intervalo de confianza al 92% para la diferencia entre probabilidad de votar por el NO de hombres y mujeres. 
...
bs_diff  <- function(x) {
  ...
  diff_p = newx$p_hat[2] - newx$p_hat[1] 
  
  return(diff_MF_p = diff_p)
}
...
diffs_bs  %>%  ggplot(aes(x=value)) + geom_density(colour="blue") + geom_vline(xintercept=ci_diffs_bs, colour="red") +
  labs(x= "Dif. en probabilidad de votar por el NO entre hombre y mujer mediana (intervalo al 92% de confianza)")
